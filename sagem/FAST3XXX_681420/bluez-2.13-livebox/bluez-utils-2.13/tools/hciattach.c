 ...
 *
 *
 *  $Id: hciattach.c,v 1.1.6.1 2005/09/25 15:02:23 shmulik Exp $
 */
...
static int read_hci_event(int fd, unsigned char* buf, int size) 
{
	...
	while (1) {
		...
		{
		    fprintf(stderr, "received unexpected char: 0x%2.2x\n",
			buf[0]);
		}
	}
	count++;
...
//Init pour les clefs usb DECT Inventel
// Vega sur usb serie en bcsp
// on ne met pas de parité par rapport au bcsp de base

static int bcsp_usb(int fd, struct uart_t *u, struct termios *ti)
{
	unsigned char byte, bcsph[4], bcspp[4],
		bcsp_sync_resp_pkt[10] = {0xc0,0x00,0x41,0x00,0xbe,0xac,0xaf,0xef,0xee,0xc0},
		bcsp_conf_resp_pkt[10] = {0xc0,0x00,0x41,0x00,0xbe,0xde,0xad,0xd0,0xd0,0xc0},
...
struct uart_t uart[] = {
	...
	{ "bcsp_usb", 0x0000, 0x0000, HCI_UART_BCSP, 115200, 115200, 0,        bcsp_usb },
